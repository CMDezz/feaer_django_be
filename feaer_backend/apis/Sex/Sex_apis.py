from feaer_backend.models import Sex
from feaer_backend.serializers import SexSerializer
...
@api_view(['GET'])
def getAll(req):
    data = Sex.objects.all()
    data_serializer = SexSerializer(data,many=True)
    return Response(data_serializer.data,status=status.HTTP_200_OK)

@api_view(['POST'])
def create(req):
    data_serializer = SexSerializer(data=req.data)
...
def deleteOne(req):
    ...
    data = Sex.objects.filter(_id = ObjectId(req.data['id'])).delete()
    if (data[0] == 0):
...
def edit(req):
    ...
    try:
        sex_obj = Sex.objects.get(_id=ObjectId(req.data['id']))
    except Sex.DoesNotExist:
        return Response({'message': 'Object not found.'}, status=status.HTTP_404_NOT_FOUND)
    data_serializer = SexSerializer(sex_obj, data=req.data, partial=True)
    if data_serializer.is_valid():
        data_serializer.save()
