from feaer_backend.models import Discount
from feaer_backend.serializers import DiscountSerializer
...
def getAll(req):
    ...
    data_serializer = DiscountSerializer(data,many=True)
    return Response(data_serializer.data,status=status.HTTP_200_OK)

@api_view(['POST'])
def create(req):
    data_serializer = DiscountSerializer(data=req.data)
...
def deleteOne(req):
    ...
    data = Discount.objects.filter(_id = ObjectId(req.data['id'])).delete()
    if (data[0] == 0):
...
def edit(req):
    ...
    try:
        Discount_obj = Discount.objects.get(_id=ObjectId(req.data['id']))
    except Discount.DoesNotExist:
        return Response({'message': 'Object not found.'}, status=status.HTTP_404_NOT_FOUND)
    data_serializer = DiscountSerializer(Discount_obj, data=req.data, partial=True)
    if data_serializer.is_valid():
        data_serializer.save()
