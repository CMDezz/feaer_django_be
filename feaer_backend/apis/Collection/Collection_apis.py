from feaer_backend.models import Collection
from feaer_backend.serializers import CollectionSerializer
...
def getAll(req):
    ...
    data_serializer = CollectionSerializer(data,many=True)
    return Response(data_serializer.data,status=status.HTTP_200_OK)

@api_view(['POST'])
def create(req):
    data_serializer = CollectionSerializer(data=req.data)
...
def deleteOne(req):
    ...
    data = Collection.objects.filter(_id = ObjectId(req.data['id'])).delete()
    if (data[0] == 0):
...
def edit(req):
    ...
    try:
        Collection_obj = Collection.objects.get(_id=ObjectId(req.data['id']))
    except Collection.DoesNotExist:
        return Response({'message': 'Object not found.'}, status=status.HTTP_404_NOT_FOUND)
    data_serializer = CollectionSerializer(Collection_obj, data=req.data, partial=True)
    if data_serializer.is_valid():
        data_serializer.save()
