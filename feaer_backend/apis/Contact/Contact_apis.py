from feaer_backend.models import Contact
from feaer_backend.serializers import ContactSerializer
...
def getAll(req):
    ...
    data_serializer = ContactSerializer(data,many=True)
    return Response(data_serializer.data,status=status.HTTP_200_OK)

@api_view(['POST'])
def create(req):
    data_serializer = ContactSerializer(data=req.data)
...
def deleteOne(req):
    ...
    data = Contact.objects.filter(_id = ObjectId(req.data['id'])).delete()
    if (data[0] == 0):
...
def edit(req):
    ...
    try:
        Contact_obj = Contact.objects.get(_id=ObjectId(req.data['id']))
    except Contact.DoesNotExist:
        return Response({'message': 'Object not found.'}, status=status.HTTP_404_NOT_FOUND)
    data_serializer = ContactSerializer(Contact_obj, data=req.data, partial=True)
    if data_serializer.is_valid():
        data_serializer.save()
